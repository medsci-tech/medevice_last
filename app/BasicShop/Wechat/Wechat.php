...
class Wechat
{
    ...
    public function messageEventCallback() {
        ...
        if (in_array($message->Content, array('产品','产品展示'))) {
            return Message::make('text')->content("<a target=\"_blank\" href=\"http://skp.ohmate.com.cn/shop\">查看优质药械产品详情</a>");
        }
        if (in_array($message->Content, array('历史','历史记录'))) {
            return Message::make('text')->content("<a target=\"_blank\" href=\"https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=MzAwMTc3MDY4NA==&scene=124\">查看历史资讯</a>");
        }
